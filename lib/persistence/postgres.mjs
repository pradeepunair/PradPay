...
export class PostgresPersistence {
  ...
  async failOutboxJob(tx, { id, owner, token, errorCode, availableAt }) {
    transaction(tx);
    for (const [value, name] of [[id,"id"],[owner,"owner"],[token,"token"],[errorCode,"errorCode"]]) required(value, name);
    const result = await tx.query(
      `UPDATE outbox_jobs SET state=CASE WHEN attempt_count>=max_attempts THEN 'dead' ELSE 'pending' END,
              available_at=COALESCE($5::timestamptz,now()),last_error_code=$4,
              lease_owner=NULL,lease_token=NULL,lease_expires_at=NULL,updated_at=now()
        WHERE id=$1 AND state='leased' AND lease_owner=$2 AND lease_token=$3 AND lease_expires_at>now() RETURNING *`,
      [id, owner, token, errorCode, availableAt ?? null],
    );
