...
export class PostgresPersistence {
  ...
  async createAcpCheckout(tx, args) {
    ...
    const document = {
      ...
      // Request line items are not response LineItem records; business resolution
      // must populate canonical response items before they can be advertised.
      line_items: [],
  ...
  async updateAcpCheckout(tx, args) {
    ...
    if (!args.input || typeof args.input !== "object" || Array.isArray(args.input)) throw new TypeError("input must be an object");
    assertSafeEventPayload(args.input, "input");
    const current = await this.retrieveAcpCheckout(tx, args);
    if (current.status !== "found") return current;
    const document = {
      ...current.checkout,
      status: "incomplete",
      // Keep canonical response LineItem records unchanged until a bounded
      // business resolver produces schema-valid replacements.
      ...(typeof args.input.order_notes === "string" ? { metadata: { note: args.input.order_notes } } : {}),
    };
    const result = await tx.query(
  ...
  async cancelAcpCheckout(tx, args) {
    ...
    const current = await this.retrieveAcpCheckout(tx, args);
    if (current.status !== "found") return current;
    const document = { ...current.checkout, status: "canceled" };
    const result = await tx.query(
      `UPDATE acp_checkout_documents SET document=$6::jsonb,version=version+1,updated_at=now()
        WHERE checkout_id=$1 AND subject=$2 AND session_id=$3 AND run_id=$4
          AND EXISTS (SELECT 1 FROM checkouts WHERE id=$1 AND acp_version=$5)
