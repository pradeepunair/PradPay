import { scheduleUnknownOutcomeReconciliation } from "../reconciliation/unknown-outcomes.mjs";
  ...
  async function admitSynthetic(claim) {
    ...
    try {
      return await persistence.withTransaction(async (tx) => {
        ...
        const reservation = await persistence.reserveSyntheticBudget(tx, claim);
  ...
  return Object.freeze({
    ...
    async executeSynthetic({ claim, provider, onCallback }) {
      ...
      try {
        ...
        const reconciliation = await scheduleUnknownOutcomeReconciliation({ persistence, operation });
        return Object.freeze({
          status: "unknown",
          admitted: true,
          admission,
    ...
    permitExistingReconciliation(operation) {
      if (operation?.status !== "unknown" || !operation.operationId || !operation.attemptId
        || !operation.sessionId || !operation.runId) {
        throw new TypeError("Existing reconciliation requires a scoped unknown operation and attempt.");
      }
      return Object.freeze({
        allowed: true,
